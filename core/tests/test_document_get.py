"""Tests for Document.getval(), SObject.get(), and top-level key tracking."""
...
def test_doc_get_multiple_exprs():
    ...
    assert doc.getval(1) == VNumber(1)
    assert doc.getval(2) == VNumber(1)
    assert doc.getval(3) is Empty


# ---------------------------------------------------------------------------
# Document.getval() — named key access
# ---------------------------------------------------------------------------

def test_doc_get_named_key():
    doc = evaluate(parse(":something (:key value)"))
    result = doc.getval("something")
...
def test_doc_get_mixed_named_and_unnamed():
    ...
    assert doc.getval("name") == VText("Joe")
    assert doc.getval(2) == VNumber(42)


# ---------------------------------------------------------------------------
# Document.getval() via REPL (merge)
# ---------------------------------------------------------------------------

def test_repl_doc_get_named():
    repl = STNRepl()
    repl.eval(":section (:title Hello)")
    result = repl.doc.getval("section")
    assert isinstance(result, VEntity)
...
def test_repl_doc_get_index_across_calls():
    ...
    repl.eval("1")
    repl.eval("2")
    assert repl.doc.getval(1) == VNumber(1)
    assert repl.doc.getval(2) == VNumber(2)
...
def test_repl_doc_get_defs_not_counted():
    ...
    assert repl.doc.getval(1) == VNumber(10)
    assert repl.doc.getval(2) is Empty
